...
## 🎯 今日目标
- [x] 掌握 Prompt Engineering 核心技巧（Few-shot / CoT / ReAct / 结构化输出）
- [x] 理解 Token 与 Context Window 机制（tiktoken 实操）
- [x] 学习 Temperature / Top-P / Stop Sequence 等参数调优
- [x] 编写 10+ 条不同场景的高质量 Prompt
...
### 3. Token 机制深入（40min）
- [x] 用 tiktoken 实测：同一段中英文的 Token 数量差异
- [x] 理解不同模型的 Tokenizer 差异（GPT vs Claude vs Qwen）
- [x] 手动计算一次 API 调用的 Token 消耗和费用
- [x] 实验：把一段文本从 100 Token 扩充到 1000 Token，观察输出质量变化
- 踩坑记录：
